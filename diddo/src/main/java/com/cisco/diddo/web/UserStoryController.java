...
public class UserStoryController {
	...
	public ResponseEntity<String> createFromJson(@RequestBody String json) {
		...
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json");
		return new ResponseEntity<String>(userStory.toJson(), headers,
				HttpStatus.CREATED);
	}
	...
	public ResponseEntity<String> getUserStoryJson(@PathVariable("id") String id) {
		...
		for (ExitCriteria exitCriteria : exitCriteriaList) {
			userStoryDetail.exitcriterias.put(exitCriteria.getDescription(),
					exitCriteria.getDone());
		}
		...
		return new ResponseEntity<String>(userStoryDetail.toJson(), headers,
				HttpStatus.OK);
	}
	...
	public ResponseEntity<String> getTasksJson(@PathVariable("id") String id) {
		...
		// String userJson = new
		// JSONSerializer().exclude("*.class").serialize(userList);
	...
	@ResponseBody
	public ResponseEntity<String> getExitCriteriasJson(
	...
	private List<ExitCriteria> findExitCriteriaByUserStory(UserStory us) {
		...
		for (ExitCriteria ec : list) {
			if (ec.getUserStory() != null
					&& ec.getUserStory().getId().equals(us.getId())) {
				conList.add(ec);
			}
		}
		return conList;
	...
	private List<Task> findTaskByUserStory(UserStory us) {
		...
		for (Task t : list) {
			if (t.getUserStory() != null
					&& t.getUserStory().getId().equals(us.getId())) {
				conList.add(t);
			}
		}
		return conList;
